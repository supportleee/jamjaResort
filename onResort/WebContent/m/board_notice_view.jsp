		...
		<div class='row'>
			<div class='col-md-12'>
				<%
					...
					String replaced_title = noticedto.getTitle();
				...
				<form method='post' enctype='multipart/form-data'>
					<table class='table'>
						<tr>
							<td>번호</td>
							<td class='two'><input type='text' name='key' id='key' style='border: 0' value='<%=noticedto.getId()%>' readonly></td>
						</tr>
						<tr>
							<td>제목</td>
						...
						<tr>
							<td class='two' colspan='2' id='button' style='text-align: right'>
								<input type='button' class='btn btn-outline-primary' value='목록' onclick="location.href='board_notice_list.jsp'"> 
							<%
 								// 세션 체크해서 있으면 관리자가 수정,삭제할 수 있도록 버튼 보이기
 								String loginOK = null;

 								loginOK = (String) session.getAttribute("login_ok");
 								if (loginOK != null && loginOK.equals("yes")) {
 							%>
 								<input class='btn btn-outline-primary' type='submit' value='수정' formaction='board_notice_update.jsp'> 
 								<input class='btn btn-outline-primary' type='button' value='삭제' onclick="location.href='board_notice_delete.jsp?key=<%=noticedto.getId()%>'"> 
 							<%
								}
							%>
							</td>
